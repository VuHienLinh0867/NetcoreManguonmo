...
namespace NETCOREMANGUONMO.Models
{
    public class StringProduct
    {
        ...
        public string GenerateKey (string id){
            ...
            string strkey = "";
            //khai bao 2 bien de chua phan so va phan chu cua tham so dau vao (id = "PRO009")
            ...
            //khai bao 1 bien kieu int = phan so tang len 1 don vi => Convert.ToInt32("009") + 1 = 10;
            int intPart = (Convert.ToInt32(numPart) + 1);
            //them cac ky tu so 0 => Do dai chuan theo quy  dinh => phan so cua ma san pham = "010"
            for (int i = 0; i < numPart.Length - intPart.ToString().Length; i++)
            {
                strPart += "0";
            }
            //ghep phan so va phan chu de tao ra ma moi ma san pham tu sinh => "PRO" + "010" = "STD010"
